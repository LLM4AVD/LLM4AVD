...
from joern_extract_tool import run_joern_analysis,run_joern_analysis_up,get_func_fullName,get_func_signature,get_interface_full_name,run_joern_analysis_inheritFunc,new_run_joern_analysis
import os
import pexpect
...
def get_filepath_by_func_name(project_path,project_name,function_name):#function_name:[MethodInvocation,func_name,code]
    ...
    joern_res=run_joern_analysis(project_path,project_name,func_name,joern_process,False)
...
def get_filepath_by_inherit_func_name(project_path,project_name,class_name,function_name):
    func_name=function_name[1]         
    # if func_name!='equals':
    #     return ''
    func_code_line=function_name[2]          
    joern_process = pexpect.spawn('joern')
    joern_process.expect('joern>', timeout=60)
    joern_res=run_joern_analysis_inheritFunc(project_path,project_name,class_name,func_name,joern_process,False)
    joern_process.sendline('exit')
    joern_process.close()
...
def get_callchain_down(project_path,
    ...
    try:
        ...
        print(f"code exists syntax bug")
        return None
    ...
    for func in functions[0]:
        ...
        if func[0].qualifier=='' and func[1] in func_in_file:
            ...
            call_chains.append(file_path+':'+func[1])
            continue
        class_name=file_path.split('/')[-1].split('.')[0]
        called_func_info=get_filepath_by_inherit_func_name(project_path,project_name,class_name,func)
        ...
        code_snippets.append(list_annotation2str(annotation)+code)
        call_chains.append(called_func_info+':'+func[1])

                       
    for func in functions[1]:
        caller_path=file_path[len(project_path)+1:]
        caller_name=function_name
        # called_func_info=get_filepath_by_func_name(project_path,project_name,func)
        called_func_info=new_get_filepath_by_func_name(project_path,project_name,caller_name,caller_path,func)
        ...
        code_snippets.append(list_annotation2str(annotation)+code)
        call_chains.append(called_func_info[0]+':'+func[1])
...
def get_call_chain_up(project_path,
    ...
    joern_process = pexpect.spawn('joern')
    joern_process.expect('joern>', timeout=60)
    up_functions=run_joern_analysis_up(project_path,project_name,function_name,joern_process,False)                        
    if up_functions==[]:
        return ([],[],[])
    ...
    func_signature=get_func_signature(project_path,project_name,file_path,function_name,joern_process,True)

    if func_signature=='':
        return ([],[],[])

    func_fullName=get_func_fullName(project_path,project_name,file_path,function_name,joern_process,True)
    func_interface_fullName=get_interface_full_name(project_path,project_name,file_path,function_name,func_fullName,joern_process,True)
    joern_process.sendline('exit')
    joern_process.close()
    func_label=[]
...
def new_get_filepath_by_func_name(project_path,project_name,caller_name,caller_path,function_name):#function_name:[MethodInvocation,func_name,code]
    ...
    joern_process = pexpect.spawn('joern')
    joern_process.expect('joern>', timeout=60)
    joern_res=new_run_joern_analysis(project_path,project_name,caller_name,callee_name,caller_path,joern_process,False)
    joern_process.sendline('exit')
    joern_process.close()
    func_signature=''
    func_called_col_number=function_name[0].position[1]
