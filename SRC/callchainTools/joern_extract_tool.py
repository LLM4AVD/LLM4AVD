...
def ensure_cpg_loaded(joern_process, cpg_path, force=False):
    try:
        loaded_path = getattr(joern_process, "_cpg_loaded_path", None)
        if force or loaded_path != cpg_path:
            import_cmd = f'importCpg("{cpg_path}")'
            joern_process.sendline(import_cmd)
            joern_process.expect('joern>', timeout=60)
            setattr(joern_process, "_cpg_loaded_path", cpg_path)
    except pexpect.exceptions.ExceptionPexpect:
        setattr(joern_process, "_cpg_loaded_path", None)
        raise

                                                    
_JOERN_PROCESS = None
_JOERN_CPG_PATH = None

def get_joern_process(cpg_path):
    global _JOERN_PROCESS, _JOERN_CPG_PATH
    if _JOERN_PROCESS is None or not _JOERN_PROCESS.isalive():
        _JOERN_PROCESS = pexpect.spawn('joern')
        _JOERN_PROCESS.expect('joern>', timeout=120)
        _JOERN_CPG_PATH = None
                         
    if _JOERN_CPG_PATH != cpg_path:
        ensure_cpg_loaded(_JOERN_PROCESS, cpg_path, force=True)
        _JOERN_CPG_PATH = cpg_path
    return _JOERN_PROCESS

def close_joern_process():
    global _JOERN_PROCESS, _JOERN_CPG_PATH
    if _JOERN_PROCESS is not None and _JOERN_PROCESS.isalive():
        try:
            _JOERN_PROCESS.sendline('exit')
            _JOERN_PROCESS.close()
        except Exception:
            pass
    _JOERN_PROCESS = None
    _JOERN_CPG_PATH = None
...
def run_joern_analysis(project_path,project_name,method_name,joern_process,cpg_path,cpg_input=False):
    try:
        ...
        if not cpg_input:
            ...
            ensure_cpg_loaded(joern_process, cpg_path)
...
def new_run_joern_analysis(project_path,project_name,caller_name,callee_name,caller_path,joern_process, cpg_path, cpg_input=False):
    try:
        if not cpg_input:
...
def run_joern_analysis_inheritFunc(project_path,project_name,class_name,method_name,joern_process,cpg_path,cpg_input=False):
    try:
...
def run_joern_analysis_up(project_path,cpg_path,method_name,joern_process,cpg_input=False):
    try:
...
def get_interface_full_name(project_path,project_name,implement_file_path,implement_func_name,func_full_name,joern_process,cpg_path,cpg_input=False):
    try:
