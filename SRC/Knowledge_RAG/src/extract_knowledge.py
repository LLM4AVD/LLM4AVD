...
#         all_before, all_after, all_modified = [], [], []
#         for ci in item.get("commits", []):
#             fname = ci.get("file", "")
#             before = ci.get("code_before_change", "") or ""
#             after = ci.get("code_after_change", "") or ""
#             modified = ci.get("function_modified_lines", {})

                                
#             all_before.append(f"\n### File: {fname}\n{before}")
#             all_after.append(f"\n### File: {fname}\n{after}")
#             all_modified.append(f"\n### File: {fname}\nAdded: {modified.get('added', [])}\nDeleted: {modified.get('deleted', [])}")

#         code_before_change = "\n".join(all_before)
#         code_after_change = "\n".join(all_after)
#         modified_lines = {"added": [], "deleted": []}
#         for ci in item.get("commits", []):
#             modified_lines["added"].extend(ci.get("function_modified_lines", {}).get("added", []))
#             modified_lines["deleted"].extend(ci.get("function_modified_lines", {}).get("deleted", []))
...
#         purpose_output = MODEL_CLIENT.generate_text(purpose_prompt_dict, args.model_settings)

#         function_prompt_dict = llm_client.generate_simple_prompt(function_prompt)
#         function_output = MODEL_CLIENT.generate_text(function_prompt_dict, args.model_settings)

#         messages = llm_client.generate_simple_prompt(analysis_prompt)
#         analysis_output = MODEL_CLIENT.generate_text(messages, args.model_settings)

#         messages.append({"role": "assistant", "content": analysis_output})
#         messages.append({"role": "user", "content": knowledge_extraction_prompt})
#         knowledge_extraction_output = MODEL_CLIENT.generate_text(messages, args.model_settings)
...
#         output_dict["modified_lines"] = modified_lines
...
#     except Exception as e:
#         print(f"Error occurred while processing CVE {item.get('cve_id')}: {str(e)}")
#         return
...
def extract_knowledge(args, item, output_data: List[Dict[str, Any]]) -> None:
    try:
        ...
        print(f"❌ Error occurred while processing item {item.get('id')}: {repr(e)}")
        return
...
def extract_knowledge_pipeline(args):
    ...
    with open(f"data/train_2/{args.input_file_name}", "r") as f:
        data = json.load(f)
