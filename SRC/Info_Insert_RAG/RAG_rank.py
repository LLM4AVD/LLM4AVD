...
def BM25_rank(search_data_path):
    corpus=[]
    f=open(search_data_path,'r',encoding='utf-8')
    lines=f.readlines()
    seg=''
    for line in lines:
        if line.strip().startswith('**') and seg!='':
            corpus.append(seg)
            seg=''
            continue
        seg+=line
    ...
    querys= [
        "注解@JalorOperation是什么，有什么功能",
        "描述@JalorOperation的功能",
        "@JalorOperation在权限检查上的功能是什么",
        "如何使用@JalorOperation",
        "介绍Jalor框架中的@JalorOperation注解的作用",
    ]
    for j,query in enumerate(querys):
        ...
        topn_indices = retriever.search(query, top_n=10)
        print(f"Sorted order of top n documents:")
        rank_result=[]
        for i,idx in enumerate(topn_indices):
            rank_result.append(corpus[idx])
            with open(f'./Rank_res/Rank{j+1}_{i+1}.txt','w',encoding='utf-8') as f:
                f.write(corpus[idx])
            print(f"Index: {idx}, Document: {corpus[idx]}")
        return rank_result
