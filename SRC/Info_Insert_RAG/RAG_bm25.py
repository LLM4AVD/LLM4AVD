...
if __name__ == "__main__":
    corpus=[]
    f=open('./JalorDoc.txt','r',encoding='utf-8')
    lines=f.readlines()
    seg=''
    for line in lines:
    ...
    for j,query in enumerate(querys):
        ...
        top5_indices = retriever.search(query, top_n=10)
        print("Sorted order of top 10 documents:")
        for i,idx in enumerate(top5_indices):
            with open(f'./Rank{j+1}_{i+1}.txt','w',encoding='utf-8') as f:
                f.write(corpus[idx])
            print(f"Index: {idx}, Document: {corpus[idx]}")